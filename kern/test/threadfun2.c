...
static void slowsafecounter(void *junk, unsigned long num) {
	int i;
	int loopBound = (int) num;
...
static void threadfun4(int nThreads, int loopBound) {
	char name[16];
	int i, result;

	for (i=0; i<nThreads; i++) {
		snprintf(name, sizeof(name), "threadtest%d", i);
		result = thread_fork(name, NULL, slowsafecounter, NULL, loopBound);
		if (result) {
			panic("threadtest: thread_fork failed %s)\n", strerror(result));
		}
	}
...
int threadtest6(int nargs, char **args) {
	...
	if (nargs != 3 && nargs != 2) {
		kprintf("Usage: tt6 [int] [optional int]");
		return 0;
	}
...
int threadtest7(int nargs, char **args) {
	...
	if (nargs != 3 && nargs != 2) {
		kprintf("Usage: tt7 [int] [optional int]");
		return 0;
	}
